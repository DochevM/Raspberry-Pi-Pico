...
void printText() {
  ...
  display.setPartialWindow(90, 10, tbw, tbh);
  display.firstPage();
  do {
    display.fillScreen(GxEPD_WHITE); 
    display.setCursor(90, 10 + tbh); 
    display.print(Text);
...
void drawImage() {
  ...
  do {
    display.fillScreen(GxEPD_WHITE);
    display.drawBitmap(0, 10, epd_bitmap_image__2_, 80, 118, GxEPD_BLACK);
  } while (display.nextPage());
}
...
//  String Humidity = String(hum) + "%";
  ...
  int16_t xTemp = 140; // Adjusted X position
  int16_t yTemp = 70;  // Lowered Y position to avoid overlapping with the label
  int16_t xHum = 140;
  int16_t yHum = 100;

  // Use a partial window to refresh only the area where the temperature and humidity are displayed
  display.setPartialWindow(xTemp, yTemp - FreeMonoBold12pt7b.yAdvance, 90/*128*/, FreeMonoBold12pt7b.yAdvance * 2 + 10);
